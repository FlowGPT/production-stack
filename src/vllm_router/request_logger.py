...
class RequestLogger:
    ...
    def enable_logging(self, enabled: bool = True, log_dir: str = None):
        ...
        if enabled:
            ...
            if not log_dir:
                log_dir = os.getcwd()
                logger.info(f"No log directory specified, will use current working directory: {log_dir}")
            ...
            try:
                ...
                log_file = os.path.join(log_dir, f"router_requests_logs_{timestamp}.csv")
                
                # Create log file and write header
                with open(log_file, 'w') as f:
                    f.write("timestamp,request_id,conversation_id,arrival_time,routing_method,target_engine,process_time\n")
    ...
    def log_request_routed(self, arrival_time: float, request_id: str, routing_method: str, target_engine: str, 
                                session_id: str = None, process_time: float = None):
